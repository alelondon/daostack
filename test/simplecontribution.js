...
const newValueSystem = require('./genesisinit').genesisInit;

let submissionFee = 1;

contract('Test SimpleContribution', function(accounts) {

    it("test Genesis scheme --> founders get their share", async function() {

      let genesis = newValueSystem.genesis;
      var controllerInstance = newValueSystem.controllerInstance;
      var reputationInstance = newValueSystem.reputationInstance;
      var tokenInstance = newValueSystem.tokenInstance;
    ...
    it("test propose SimpleContribution scheme -> approve --> submitContribution --> submission fee", async function(){
      ...
      // propose SimpleContribution

      contributionScheme = await SimpleContribution.new(controllerInstance, submissionFee);

      await genesis.proposeScheme(contributionScheme.address)

      await genesis.voteScheme(contributionScheme.address, true, {from: accounts[0]});
      let voteStatus = gensis.getVoteStatus(contributionScheme.address);
      assert.equal(voteStatus, [7, 0, 1, 0] , "vote status is not as expected");

      await genesis.voteScheme(contributionScheme.address, false, {from: accounts[2]});
      let voteStatus = genesis.getVoteStatus(contributionScheme.address);
      assert.equal(voteStatus, [7, 12, 1, 0] , "vote status is not as expected");

      await genesis.voteScheme(contributionScheme.address, true, {from: accounts[1]});
      let voteStatus = genesis.getVoteStatus(contributionScheme.address);
      let voteResult = genesis.voteResults(contributionScheme.address);
      assert.equal(voteStatus, [16, 12, 1, 1] , "vote status is not as expected");
      assert.equal(voteResult, true , "vote result is not as expected");
      assert.equal(Controller.schemes(contributionScheme.address), true, "scheme was not registered");
    };
